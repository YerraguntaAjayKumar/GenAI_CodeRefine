        ...
        if tot_rev == 0:
            ...
            if all_h:
                ...
                df2 = pd.DataFrame(all_h).rename(columns={
                    ...
                    "orig_time_complexity":"Original O(·)","rw_time_complexity":"Rewritten O(·)","created_at":"Date",
                })
                df2["Date"] = df2["Date"].str[:16]
                df2 = df2.drop(columns=["original_code","orig_complexity_score","rw_complexity_score"],errors="ignore")
                cols_order = ["Date","Language","Bugs","Security","Perf %","Confidence %","Complexity","Original O(·)","Rewritten O(·)"]
